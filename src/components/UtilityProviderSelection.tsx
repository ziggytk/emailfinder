...
export const UtilityProviderSelection: React.FC<UtilityProviderSelectionProps> = ({ 
  ...
  const [propertyToDelete, setPropertyToDelete] = useState<Address | null>(null);
  ...
  const handleRemoveProperty = (address: Address) => {
    setPropertyToDelete(address);
  ...
  const confirmDeleteProperty = () => {
    ...
    // Remove all accounts for this address
    const updated = selectedAccounts.filter(account => 
      !(account.address.street === propertyToDelete.street &&
        account.address.city === propertyToDelete.city &&
        account.address.state === propertyToDelete.state &&
        account.address.zipCode === propertyToDelete.zipCode)
    );
  ...
  const handleClearSelection = () => {
    // Don't allow clearing all properties - at least one should remain
    if (selectedAccounts.length > 0) {
      // Keep the first property (or you could show a confirmation dialog)
      const firstProperty = selectedAccounts[0];
      const firstPropertyAccounts = selectedAccounts.filter(account => 
        account.address.street === firstProperty.address.street &&
        account.address.city === firstProperty.address.city &&
        account.address.state === firstProperty.address.state &&
        account.address.zipCode === firstProperty.address.zipCode
      );
      
      setSelectedAccounts(firstPropertyAccounts);
      onAccountsSelected(firstPropertyAccounts);
    }
  };
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              title="Remove all properties except the first one"
            >
                  ...
                  <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {/* Delete Property Button - Only show if there's more than one property */}
                        {getUniqueAddresses().length > 1 && (
                          <button
                            onClick={() => handleRemoveProperty(address)}
                            ...
                            title="Delete this property and all its utility accounts"
                          >
                            <X className="w-4 h-4" />
                            Delete Property
                          </button>
          ...
          <div className="bg-white rounded-lg w-full max-w-md">
            ...
            <div className="p-6">
              <p className="text-gray-700 mb-4">
                Are you sure you want to delete this property and all its utility accounts?
              </p>
              <div className="bg-gray-50 p-4 rounded-lg mb-6">
                <p className="font-medium text-gray-900">{propertyToDelete.street}</p>
                <p className="text-sm text-gray-600">
                  {propertyToDelete.city}, {propertyToDelete.state} {propertyToDelete.zipCode}
                </p>
              </div>
              <p className="text-sm text-red-600 mb-6">
                This action cannot be undone. All utility accounts for this property will be permanently removed.
              </p>
